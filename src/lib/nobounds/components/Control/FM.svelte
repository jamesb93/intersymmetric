<script>
    ...
    import { pitch4, pitch5, shape4, shape5, length, pitchOffset } from '$lib/nobounds/firebase-core';
    ...
    $: instrument.fund.rampTo($params[id].frequency * freqMap($pitchOffset + $pitch), 0.1);
    ...
    $: instrument.c1env.release = $params[id].c1release * ($length * Math.max($shape, 0.01));
    $: instrument.c2env.release = $params[id].c2release * ($length * Math.max($shape, 0.01));
    $: instrument.c3env.release = $params[id].c3release * ($length * Math.max($shape, 0.01));
