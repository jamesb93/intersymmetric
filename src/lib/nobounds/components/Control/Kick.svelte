<script>
    ...
    $: {
        let calculatedFreq = Math.max(0, $params.kick.frequency * freqMap($pitchOffset + $pitch0));
    ...
    $: kick.membrane.envelope.attack = Math.max($params.kick.attack * $length * Math.max($shape0, 0.01), 0);
    $: kick.membrane.envelope.sustain = clip($params.kick.sustain, 0.0, 1.0);

    $: kick.membrane.envelope.decay = Math.max($params.kick.decay * $length * Math.max($shape0, 0.01), 0);
    $: kick.membrane.envelope.release = Math.max($params.kick.release * $length * Math.max($shape0, 0.01), 0);
