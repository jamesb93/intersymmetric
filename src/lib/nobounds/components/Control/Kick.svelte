<script>
  ...
  $: kick.membrane.envelope.decay = Math.max($params.kick.decay * $length * Math.max($trackShape[0], 0.01), 0);
  $: kick.membrane.envelope.release = Math.max($params.kick.release * $length * Math.max($trackShape[0], 0.01), 0);
  $: kick.distortion.distortion = clip($params.kick.distortion, 0.0, 1.0);

  $: {
    let calculatedFreq = Math.max(0, $params.kick.frequency * freqMap($pitchOffset + $trackPitch[0]));
