<script>
  ...
  import { socket, grid, gridValid, euclidSteps, trackPitch, trackSound, trackShape, params } from '$lib/nobounds/app';
  ...
  function updateSound(x) {
    let presets;
    if (x === 0) presets = kickPreset;
    if (x === 1) presets = snarePreset;
    if (x === 2) presets = metal1Preset;
    if (x === 3) presets = metal2Preset;
    if (x === 4) presets = fm1Preset;
    if (x === 5) presets = fm2Preset;
    ...
    let result = d3.interpolateObject(presets[preset1], presets[preset2])(amount);
    ...
    <div class="euclids">
      ...
      {#each { length: 6 } as _, x}
        <Knob
          ...
          func={() => {
            updateSound(x);
            socket.emit('trackSound', $trackSound);
          }}
        />
      {/each}
    </div>
