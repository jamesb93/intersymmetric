<script>
	...
	import {
		...
		numLoadedSamples,
		totalNumSamples
	} from '$lib/nobounds/samplerConfig.js';
	...
	function sendPlaybackRate() {
		socket.emit('playbackRate', $playbackRate);
	}
	...
	onMount(async () => {
		...
		loop = new Tone.Loop((time) => {
			...
			if ($grid[1][pos] === true)
				// samplers[0].trigger(time, 1, 1.0, 10.0)
				samplers[1].trigger(time, $sampleSelectors[1], 1.0, $length * $trackLengths[1]);

			if ($grid[2][pos] === true)
				// samplers[0].trigger(time, 1, 1.0, 10.0)
				samplers[2].trigger(time, $sampleSelectors[2], 1.0, $length * $trackLengths[2]);

			if ($grid[3][pos] === true)
				// samplers[0].trigger(time, 1, 1.0, 10.0)
				samplers[3].trigger(time, $sampleSelectors[3], 1.0, $length * $trackLengths[3]);

			if ($grid[4][pos] === true)
				// samplers[0].trigger(time, 1, 1.0, 10.0)
				samplers[4].trigger(time, $sampleSelectors[4], 1.0, $length * $trackLengths[4]);

			if ($grid[5][pos] === true)
				// samplers[0].trigger(time, 1, 1.0, 10.0)
				samplers[5].trigger(time, $sampleSelectors[5], 1.0, $length * $trackLengths[5]);
